...
function platformRuntimeOnLibraryChanged(handler) {
  if (!handler || typeof handler !== "function") return;
  const fromShared = platformOpsCallShared(
    "call",
    "runtime.onLibraryChanged",
    [handler],
    {
      capability: "eagleApi",
      operationLabel: "runtime.onLibraryChanged",
    },
    null,
  );
  if (fromShared !== undefined) {
  ...
  try {
    if (
      platform &&
      platform.runtime &&
      typeof platform.runtime.onLibraryChanged === "function"
    ) {
      platform.runtime.onLibraryChanged(handler);
      return;
    }
  } catch (_) {}
  platformWarnMissingCapability("eagleApi", "runtime.onLibraryChanged");
}
...
async function platformWindowClose() {
  // Desktop Electron : l'adaptateur expose close (via le shim preload)
  // Eagle reel : close absent de l'adaptateur, fallback sur hide
  const platform = getPlatformAdapter();
  if (platform?.window?.close) {
    try {
...
const PREF_KEY_DRAWING_PAUSE_MODES = "drawingPauseModes";
...
async function openDrawingModeSafely() {
  ...
  if (root && typeof root.openDrawingMode === "function") {
    // Détermine si l'ouverture du module dessin doit déclencher une pause,
    // selon les préférences par mode (PREF_KEY_DRAWING_PAUSE_MODES).
    const pauseModes = UIPreferences.get(PREF_KEY_DRAWING_PAUSE_MODES, [
      "classique",
      "custom",
      "relax",
      "memory",
    ]);
    const currentMode = String(state.sessionMode || "classique").trim();
    const shouldPause =
      Array.isArray(pauseModes) && pauseModes.includes(currentMode);
    window.__poseChronoDrawPause = shouldPause;
    root.openDrawingMode();
    // Nettoyage immédiat : le flag est lu de façon synchrone par draw.js
    delete window.__poseChronoDrawPause;
    return true;
  }
  return false;
}
...
const UIPreferences = SHARED_UI_PREFERENCES_FACTORY
  ? SHARED_UI_PREFERENCES_FACTORY({
      ...
      defaults: {
        ...
        soundPause: true,
        drawingPauseModes: ["classique", "custom", "memory"],
...
async function resolveSessionMediaSelection() {
  ...
  const result = await SESSION_MEDIA_UTILS.resolveMediaSelection({
    getSelectedItems: async () => {
      const items = await platformItemGetSelected();
      return items;
    },
    getSelectedFolders: () =>
    ...
    queryItems: async (query) => {
      // La méthode native item.get({ folders: [...] }) est buggée et retourne toujours 0 résultat.
      // Contournement : on récupère tous les items et on filtre côté client.
      if (Array.isArray(query?.folders) && query.folders.length > 0) {
        const folderIdSet = new Set(query.folders);
        const allItems = await platformItemGet({});
        if (!Array.isArray(allItems)) return [];
        return allItems.filter(
          (item) =>
            Array.isArray(item?.folders) &&
            item.folders.some((fId) => folderIdSet.has(fId)),
        );
      }
      return platformItemGet(query);
    },
    toFolderIds: (folders) =>
...
const GLOBAL_SETTINGS_ACTIONS = {
  ...
  drawingPauseModes: {
    id: "global-settings-drawing-pause-modes-group",
    control: "modes-toggle-multi",
    i18nKey: "settings.global.drawingPauseModes",
    fallback: "Pause when opening drawing module",
    tooltipKey: "settings.global.drawingPauseModesTooltip",
    span: "full",
  },
  exportPrefs: {
...
const GLOBAL_SETTINGS_SECTIONS = [
  ...
  {
    id: "dessin",
    titleKey: "settings.global.drawingSection",
    fallbackTitle: "Drawing",
    icon: "",
    actionGroups: [
      {
        columns: 1,
        align: "start",
        items: [{ type: "action", key: "drawingPauseModes" }],
      },
    ],
  },
  {
...
function renderGlobalSettingsSections() {
  ...
  const buildActionsElement = (groupConfig) => {
    ...
    (normalizedGroup.items || []).forEach((item) => {
      ...
      if (actionConfig.control === "modes-toggle-multi") {
        // Sélection multiple : chaque mode peut être activé/désactivé indépendamment
        const multiRow = document.createElement("div");
        multiRow.className = `global-settings-mode-toggle-row ${
          actionConfig.className || ""
        } ${item.className || ""}`.trim();

        const multiLabel = document.createElement("span");
        multiLabel.className =
          "global-settings-mode-label hotkey-description";
        if (actionConfig.i18nKey) {
          multiLabel.setAttribute("data-i18n", actionConfig.i18nKey);
        }
        if (actionConfig.tooltipKey) {
          multiLabel.setAttribute("data-i18n-tooltip", actionConfig.tooltipKey);
        }
        multiLabel.textContent = getGlobalSettingsText(
          actionConfig.i18nKey || "",
          actionConfig.fallback || "",
        );
        ...
        multiGroup.id = actionConfig.id;
        multiGroup.className =
          "hotkeys-search-toggle global-settings-mode-toggle global-settings-mode-toggle-multi";
        multiGroup.setAttribute("role", "group");
        multiGroup.setAttribute("aria-label", multiLabel.textContent);

        const sessionModesMT = [
          {
            value: "classique",
            key: "modes.classic.title",
            fallback: "Classic",
          ...
          {
            value: "memory",
            key: "modes.memory.title",
            fallback: "Memory",
          },
        ];

        // Lire les modes actifs depuis les préférences
        const activePauseModes = UIPreferences.get(
          PREF_KEY_DRAWING_PAUSE_MODES,
          ["classique", "custom", "relax", "memory"],
        );

        sessionModesMT.forEach((modeConfig) => {
          const btn = document.createElement("button");
          btn.type = "button";
          btn.className = "search-toggle-btn";
          btn.dataset.mode = modeConfig.value;
          ...
          const isActive =
            Array.isArray(activePauseModes) &&
            activePauseModes.includes(modeConfig.value);
          btn.classList.toggle("active", isActive);
          btn.setAttribute("aria-pressed", isActive ? "true" : "false");

          btn.addEventListener("click", () => {
            const currentModes = UIPreferences.get(
              PREF_KEY_DRAWING_PAUSE_MODES,
              ["classique", "custom", "relax", "memory"],
            );
            const modeArr = Array.isArray(currentModes) ? [...currentModes] : [];
            const idx = modeArr.indexOf(modeConfig.value);
            if (idx === -1) {
              modeArr.push(modeConfig.value);
            ...
            UIPreferences.set(PREF_KEY_DRAWING_PAUSE_MODES, modeArr);
          });

          multiGroup.appendChild(btn);
        });

        multiRow.appendChild(multiGroup);
        applyGridSpan(multiRow, item, actionConfig);
        actionsEl.appendChild(multiRow);
        return;
      }
...
function updateGlobalSettingsModalState() {
  ...
  const drawingPauseModesGroup = document.getElementById(
    "global-settings-drawing-pause-modes-group",
  );
  if (drawingPauseModesGroup) {
    const activePauseModes = UIPreferences.get(PREF_KEY_DRAWING_PAUSE_MODES, [
      "classique",
      "custom",
      "relax",
      "memory",
    ]);
    drawingPauseModesGroup
      .querySelectorAll(".search-toggle-btn[data-mode]")
      .forEach((btn) => {
        const isActive =
          Array.isArray(activePauseModes) &&
          activePauseModes.includes(btn.dataset.mode);
        btn.classList.toggle("active", isActive);
        btn.setAttribute("aria-pressed", isActive ? "true" : "false");
      });
  }
...
let syncParticipantLastDownloadedPackHash = null; // hash du dernier pack téléchargé par le guest
...
async function killEagleLocalSyncServer() {
  ...
  if (_eagleLocalSyncProcess) {
    if (_eagleLocalSyncProcess.inProcess) {
      ...
      try { _eagleLocalSyncProcess.kill(); } catch (_) {}
    }
...
// Retourne le nombre d'images chargées dans la session courante (state.images de l'app,
// distinct de syncSessionServiceState). Utilisé pour le warning limite 300 côté hôte.
function getAppImagesCount() {
...
function renderSyncSessionStatus(modal, snapshot = null) {
  ...
  if (
    SYNC_SESSION_STATUS_UI &&
    typeof SYNC_SESSION_STATUS_UI.updatePackStatusUi === "function"
  ) {
    SYNC_SESSION_STATUS_UI.updatePackStatusUi(modal, state, {
      getText: getI18nText,
      totalMediaCount: getAppImagesCount(),
      downloadedPackHash: syncParticipantLastDownloadedPackHash,
    });
  }
...
async function collectSyncSessionMediaUploadEntries(pack) {
  // Mélange aléatoire (Fisher-Yates) avant sélection : quand il y a plus d'images que la
  // limite (SYNC_SESSION_MEDIA_MAX_FILES = 300), les 300 uploadées sont choisies aléatoirement
  // parmi toutes les refs disponibles, plutôt que systématiquement les N premières.
  const rawRefs = Array.isArray(pack?.mediaRefs) ? pack.mediaRefs : [];
  const refs = rawRefs.slice();
  for (let s = refs.length - 1; s > 0; s -= 1) {
...
function applyRemoteSyncRuntimeState(remoteState) {
  ...
  try {
    ...
    if (typeof remoteState.memoryHidden === "boolean") {
      ...
      // Synchroniser le DOM de l'overlay avec l'état reçu de l'hôte.
      // Sans cet appel, le participant met bien à jour state.memoryHidden
      // mais l'overlay reste bloqué dans son état DOM précédent.
      if (state.memoryHidden) {
        showMemoryOverlay();
      } else {
        hideMemoryOverlay();
      }
    ...
    if (remoteSessionActive === true && requiresPackValidationForStart) {
      ...
      stopTimer();
      // Fermer le mode dessin s'il est actif (même logique que showReview)
      if (
        typeof closeDrawingMode === "function" &&
        typeof isDrawingModeActive !== "undefined" &&
        isDrawingModeActive
      ...
      // Nettoyer les badges/classes de pause (même logique que showReview)
      if (timerDisplay) timerDisplay.classList.remove("timer-paused");
      if (pauseBadge) pauseBadge.classList.add("hidden");
      // Réinitialiser l'overlay mémoire si visible (même logique que showReview)
    ...
    // Résoudre l'image courante par mediaKey (identité) plutôt que par index brut.
    // Cela garantit que host et guest affichent la même image même si leurs
    // tableaux state.images ont des tailles différentes (ex. 303 vs 300).
    let resolvedIndex = -1;
    const remoteMediaKey = String(remoteState.currentMediaKey || "").trim();
    if (
      remoteMediaKey &&
      Array.isArray(state.images) &&
      state.images.length > 0
      ...
      for (let i = 0; i < state.images.length; i++) {
        if (getSyncMediaIdentity(state.images[i]) === remoteMediaKey) {
          resolvedIndex = i;
          break;
        }
      }
    ...
    if (resolvedIndex >= 0 && Array.isArray(state.images) && state.images.length > 0) {
      const clampedIndex = Math.max(
        0,
        Math.min(resolvedIndex, state.images.length - 1),
      );
      // Réinitialiser le flip sur changement d'image (comme nextImage côté hôte).
      // On ne réplique pas le state.timeRemaining de resetTransforms() car il est
      // déjà synchronisé depuis remoteState plus haut dans cette fonction.
    ...
    // Synchroniser l'interface des réglages de l'invité (durées, type mémoire, etc.)
    syncGuestSettingsContainerUi();
  } catch (error) {
...
/**
 * Met à jour l'interface des réglages de session pour l'invité sync,
 * après réception d'un état depuis l'hôte. Reflète visuellement le mode
 * actif et tous les paramètres sans impacter la session locale.
 * Exclut les préférences personnelles (#home-progressive-blur-btn, #autoflip-btn).
 */
function syncGuestSettingsContainerUi() {
  // --- Boutons de sélection de mode ---
  // Mise à jour explicite nécessaire : switchMode() n'est appelé que si le mode
  // change, donc à la connexion initiale (même mode que le défaut), les boutons
  // ne seraient pas mis à jour sans ce bloc.
  document.querySelectorAll(".mode-btn").forEach((btn) => {
    btn.classList.toggle("active", btn.dataset.mode === state.sessionMode);
  });

  // --- Mode classique : boutons de durée + inputs H:M:S ---
  const classicPanel = document.getElementById("mode-classique-settings");
  if (classicPanel) {
    if (typeof syncClassicDurationButtons === "function") {
      syncClassicDurationButtons(classicPanel);
    }
    const durSec = Math.max(
      0,
      Math.floor(Number(state.selectedDuration || 0) || 0),
    );
    const hEl = document.getElementById("hours-input");
    const mEl = document.getElementById("minutes-input");
    const sEl = document.getElementById("seconds-input");
    if (hEl) hEl.value = String(Math.floor(durSec / 3600));
    if (mEl) mEl.value = String(Math.floor((durSec % 3600) / 60));
    if (sEl) sEl.value = String(Math.floor(durSec % 60));

    // Highlight de la section durée personnalisée (H:M:S) :
    // actif si aucun bouton preset ne correspond ET qu'une durée custom est définie
    const anyPresetActive = Array.from(
      classicPanel.querySelectorAll(".duration-btn"),
    ).some((btn) => btn.classList.contains("active"));
    DOMCache.inputGroups.forEach((g) => {
      g.classList.toggle("active", !anyPresetActive && state.selectedDuration > 0);
    });
  }

  // --- Mode mémoire : type flash / progressif ---
  const memType = String(state.memoryType || "flash").trim();
  // Boutons de type (flash / progressif)
  if (Array.isArray(memoryTypeBtns)) {
    memoryTypeBtns.forEach((btn) => {
      const btnType = String(btn.dataset?.memoryType || "").trim();
      const isActive = btnType === memType;
      btn.classList.toggle("active", isActive);
      btn.setAttribute("aria-pressed", isActive ? "true" : "false");
    });
  }
  // Visibilité des sous-panneaux flash / progressif
  // Utilisation de style.display (cohérent avec bindMemoryTypeSwitchButtons)
  // pour éviter le conflit avec le style.display="none" inline initial du panneau progressif.
  if (memoryFlashSettings) {
    memoryFlashSettings.style.display = memType === "flash" ? "block" : "none";
  }
  if (memoryProgressiveSettings) {
    memoryProgressiveSettings.style.display =
      memType === "progressive" ? "block" : "none";
  }

  // Boutons de durée mémoire
  if (typeof syncMemoryDurationButtons === "function") {
    syncMemoryDurationButtons();
  }

  // Highlight des sections de durée custom mémoire (.memory-custom-time)
  // À faire APRÈS syncMemoryDurationButtons() qui met à jour les boutons preset
  if (memoryFlashSettings) {
    const flashCustomTime =
      memoryFlashSettings.querySelector(".memory-custom-time");
    const anyFlashPresetActive = Array.from(
      memoryFlashSettings.querySelectorAll(".duration-btn"),
    ).some((btn) => btn.classList.contains("active"));
    if (flashCustomTime) {
      flashCustomTime.classList.toggle(
        "active",
        !anyFlashPresetActive && state.memoryDuration > 0,
      );
    }
  }
  if (memoryProgressiveSettings) {
    const progCustomTime = memoryProgressiveSettings.querySelector(
      ".memory-custom-time",
    );
    const anyProgPresetActive = Array.from(
      memoryProgressiveSettings.querySelectorAll(".duration-btn"),
    ).some((btn) => btn.classList.contains("active"));
    if (progCustomTime) {
      progCustomTime.classList.toggle(
        "active",
        !anyProgPresetActive && state.selectedDuration > 0,
      );
    }
  }

  // Inputs de durée mémoire (flash et progressif partagent state.memoryDuration)
  const memDurSec = Math.max(
    0,
    Math.floor(Number(state.memoryDuration || 0) || 0),
  );
  const memDurMin = Math.floor(memDurSec / 60);
  const memDurRem = memDurSec % 60;
  const flashMinEl = document.getElementById("memory-flash-minutes");
  const flashSecEl = document.getElementById("memory-flash-seconds");
  const progMinEl = document.getElementById("memory-progressive-minutes");
  const progSecEl = document.getElementById("memory-progressive-seconds");
  if (flashMinEl) flashMinEl.value = String(memDurMin);
  if (flashSecEl) flashSecEl.value = String(memDurRem);
  if (progMinEl) progMinEl.value = String(memDurMin);
  if (progSecEl) progSecEl.value = String(memDurRem);

  // Inputs du temps de dessin mémoire
  const drawTimeSec = Math.max(
    0,
    Math.floor(Number(state.memoryDrawingTime || 0) || 0),
  );
  const drawMinEl = document.getElementById("memory-drawing-minutes");
  const drawSecEl = document.getElementById("memory-drawing-seconds");
  if (drawMinEl) drawMinEl.value = String(Math.floor(drawTimeSec / 60));
  if (drawSecEl) drawSecEl.value = String(drawTimeSec % 60);

  // Highlight de la section temps de dessin (#memory-drawing-time-input)
  // Actif si une durée est définie ET que le mode "sans pression" n'est pas activé
  const memoryDrawingTimeInput = document.getElementById(
    "memory-drawing-time-input",
  );
  if (memoryDrawingTimeInput) {
    memoryDrawingTimeInput.classList.toggle(
      "active",
      state.memoryDrawingTime > 0 && !state.memoryNoPressure,
    );
  }

  // Bouton "sans pression" (mémoire)
  const noPressureBtn = document.getElementById("no-pressure-btn");
  if (noPressureBtn) {
    noPressureBtn.classList.toggle("active", !!state.memoryNoPressure);
    noPressureBtn.setAttribute(
      "aria-pressed",
      state.memoryNoPressure ? "true" : "false",
    );
  }

  // Compteurs de poses mémoire (flash et progressif)
  const memPoses = Math.max(1, Number(state.memoryPosesCount || 1));
  const flashSlider = document.getElementById("memory-poses-slider");
  const flashPosesValue = document.getElementById("memory-poses-value");
  const progSlider = document.getElementById("memory-progressive-poses-slider");
  const progPosesValue = document.getElementById(
    "memory-progressive-poses-value",
  );
  if (flashSlider) flashSlider.value = String(memPoses);
  if (flashPosesValue) flashPosesValue.textContent = String(memPoses);
  if (progSlider) progSlider.value = String(memPoses);
  if (progPosesValue) progPosesValue.textContent = String(memPoses);

  // Durée totale mémoire (#memory-total-duration)
  const memTotalDur = document.getElementById("memory-total-duration");
  const memTotalDurVal = document.getElementById("memory-total-duration-value");
  if (memTotalDur && memTotalDurVal) {
    if (SESSION_CONTROLS_BINDINGS_UTILS?.resolveMemoryTotalDurationDisplay) {
      ...
      if (viewModel.visible) {
        memTotalDurVal.textContent = viewModel.text;
        memTotalDur.style.display = "block";
      } else {
        memTotalDur.style.display = "none";
      }
    }
  }
}
...
/**
 * Construit le HTML du statut avec des "..." animés (réutilise l'animation CSS
 * sync-session-status-loading-dot déjà définie pour #sync-session-network-status).
 * Retire les "..." ou "…" de fin du texte traduit pour les remplacer par les spans animés.
 */
function buildSyncLoadingStatusHtml(text) {
  const base = String(text || "").replace(/\.{3}$|…$/, "").trimEnd();
  // Échappement HTML minimal (le texte vient de i18n, pas de saisie utilisateur)
  const escaped = base
    .replace(/&/g, "&amp;")
    .replace(/</g, "&lt;")
    .replace(/>/g, "&gt;");
...
function updateSyncTransferModalState(options = {}) {
  ...
  if (status && elements.statusEl) {
    if (loading) {
      // Points animés : réutilise les spans déjà présents pour ne pas réinitialiser
      // l'animation CSS à chaque callback (ex : upload fichier par fichier).
      const existingDots = elements.statusEl.querySelector(
        ".sync-session-status-loading-dots",
      );
      if (existingDots) {
        // Met à jour uniquement le nœud texte avant les dots — sans toucher les spans animés
        const base = String(status).replace(/\.{3}$|…$/, "").trimEnd();
        const firstChild = elements.statusEl.firstChild;
        if (firstChild && firstChild.nodeType === Node.TEXT_NODE) {
          firstChild.textContent = base;
        } else {
          elements.statusEl.insertBefore(
            document.createTextNode(base),
            existingDots,
          );
        }
      } else {
        // Première fois : créer la structure complète avec les dots animés
        elements.statusEl.innerHTML = buildSyncLoadingStatusHtml(status);
      }
    } else {
...
function setupSyncSessionModalBindings() {
  ...
  if (startLocalServerBtn) {
    ...
    const healthInterval = setInterval(() => {
      if (localServerStoppedManually) return;
    ...
    startLocalServerBtn.addEventListener("click", async () => {
      ...
      if (window.poseChronoDesktop?.sync?.startLocalServer) {
        ...
        try {
          startResult = await window.poseChronoDesktop.sync.startLocalServer();
        } catch (e) {
          console.warn("[Sync] Failed to start local server via IPC:", e);
          startResult = { ok: false, error: String(e && e.message ? e.message : e) };
        }
        if (!startResult || startResult.ok !== true) {
          console.warn("[Sync] Desktop local server start failed:", startResult);
          showSyncSessionToast({
            type: "error",
            message: getI18nText(
              "sync.localServerStartFailed",
        ...
        // Mode Eagle : lancement silencieux du serveur sync local
        // Eagle a Node.js integre dans le renderer, on peut spawner node directement
        // sans ouvrir de terminal visible ni d'explorateur de fichiers.
        try {
          const urlObj = new URL(window.location.href);
          let basePath = decodeURIComponent(urlObj.pathname);
          basePath = basePath.replace(/^\/+([a-zA-Z]:\/)/, '$1');
          const pluginRoot = basePath.substring(0, basePath.lastIndexOf('/'));
          const serverScript = pluginRoot + '/scripts/sync-relay-server.js';

          const navPlatform = String(navigator?.platform || "").toLowerCase();
          const navUA = String(navigator?.userAgent || "").toLowerCase();
          const isWindows = navPlatform.includes("win") || navUA.includes("windows");
          const normalizePathForPlatform = (p) =>
            isWindows ? p.replace(/\//g, '\\') : p;
          ...
          // Priorite 2 : spawn node en child process (si require echoue)
          if (!launched) {
            try {
              ...
              child.on('error', (err) => {
                console.warn("[Sync] child_process spawn error:", err);
                if (_eagleLocalSyncProcess === child) _eagleLocalSyncProcess = null;
              });
              child.on('exit', (code) => {
                if (code !== 0 && code !== null) {
                  console.warn("[Sync] local server exited with code:", code);
                }
                if (_eagleLocalSyncProcess === child) _eagleLocalSyncProcess = null;
              });
              _eagleLocalSyncProcess = child;
              launched = true;
            } catch (spawnErr) {
              console.warn("[Sync] child_process spawn failed:", spawnErr);
            }
          }

          // Priorite 2 : fallback openPath sur le .bat/.sh (terminal visible)
          if (!launched) {
            const platform = getPlatformAdapter();
            if (platform?.system?.openPath) {
              const isMac = navPlatform.includes("mac") || navUA.includes("mac os");
              const launchersDir = pluginRoot + '/js/syncroModule';
              const primaryLauncher =
                launchersDir +
                (isWindows
                  ? '/start_local_sync_server.bat'
                  : isMac
                    ? '/start_local_sync_server.command'
                    : '/start_local_sync_server.sh');
              await platform.system.openPath(normalizePathForPlatform(primaryLauncher));
            }
          }
        } catch (e) {
          console.warn("[Sync] Failed to launch local server:", e);
        }
      }
        ...
        if (health.ok) {
          ...
          showSyncSessionToast({
            ...
            message:
              ...
              " (" + getI18nText("sync.ipCopied", "IP copiée") + " : " +
              startedAddressLabel +
              ")",
            duration: 4000,
          });
      ...
      // Auto-fill workflow: poll clipboard for the new IP pasted by the launcher script
  ...
  const handlePublishOnlinePackClick = async () => {
    ...
    try {
      ...
      const pack = buildSyncSessionPackManifest();
      // 1er publish : config de session uniquement, sans les refs media (pas encore uploadées).
      // Évite qu'un participant qui rejoint pendant l'upload récupère un pack stale avec plus
      // de refs que de fichiers réellement disponibles (ex. "300/313 - incomplet").
      const packConfigOnly = { ...pack, mediaRefs: [] };
  ...
  const handleDownloadOnlinePackClick = async () => {
    ...
    try {
      ...
      // expectedMediaFiles = ce que le serveur a réellement stocké (vérité serveur).
      // On n'utilise plus pack.mediaRefs.length : ce chiffre peut être stale si le participant
      // a reçu le pack avant le 2ème publish (effectivePack). Le serveur fait autorité.
      ...
      // Mémoriser le hash pour afficher "téléchargé" au lieu de "disponible" dans la barre pack
      if (remoteHash) syncParticipantLastDownloadedPackHash = remoteHash;
      syncParticipantTransferInProgress = false;
...
// Quand l'utilisateur change de librairie Eagle pendant une session, les chemins
// file:// de l'ancienne librairie deviennent inaccessibles → on arrête la session
// et on retourne aux paramètres pour recharger les images de la nouvelle librairie.
platformRuntimeOnLibraryChanged(() => {
  stopTimer();
  const drawingScreenEl =
    drawingScreen || document.getElementById("drawing-screen");
  const settingsScreenEl =
    settingsScreen || document.getElementById("settings-screen");
  const reviewScreenEl =
    reviewScreen || document.getElementById("review-screen");
  if (drawingScreenEl) drawingScreenEl.classList.add("hidden");
  if (reviewScreenEl) {
    reviewScreenEl.classList.add("hidden");
    document.body.classList.remove("review-active");
  }
  if (settingsScreenEl) settingsScreenEl.classList.remove("hidden");
  void loadImages();
});
/**
...
function setupEventListeners() {
  ...
  const handleResetAllSettings = async () => {
    ...
    if (UIPreferences.setStringArray) {
      ...
      UIPreferences.setStringArray(PREF_KEY_DRAWING_PAUSE_MODES, [
        "classique",
        "custom",
        "memory",
      ]);
    ...
    applySidebarVisibility();
    globalSettingsCollapsed.clear();
    ["maintenance", "son", "appearance", "dessin"].forEach((k) => globalSettingsCollapsed.add(k));
    saveGlobalSettingsCollapsedState();
  ...
  // === SYNC HÔTE : propagation temps réel des réglages aux invités ===
  // Quand l'hôte modifie durée / type mémoire / inputs dans settings-screen,
  // on schedule immédiatement un envoi de l'état runtime à tous les participants.
  if (settingsScreen) {
    settingsScreen.addEventListener("click", (e) => {
      if (!isSyncSessionHostActive()) return;
      if (
        e.target.closest(
          ".duration-btn, .memory-type-btn, #no-pressure-btn",
        )
      ) {
        scheduleSyncRuntimeState("settings-user-interaction", { force: true });
      }
    });
    settingsScreen.addEventListener("change", () => {
      if (!isSyncSessionHostActive()) return;
      scheduleSyncRuntimeState("settings-user-interaction", { force: true });
    });
    // Real-time sync des inputs de temps personnalisé (H:M:S, mémoire, etc.)
    // L'événement "input" se déclenche à chaque frappe, on debounce pour ne
    // pas spammer le réseau tout en gardant une mise à jour live chez l'invité.
    const debouncedInputSync = PerformanceUtils.debounce(() => {
      if (!isSyncSessionHostActive()) return;
      scheduleSyncRuntimeState("settings-input-live", { force: true });
    }, 250);
    settingsScreen.addEventListener("input", (e) => {
      if (!isSyncSessionHostActive()) return;
...
function switchMode(mode) {
  ...
  if (transitionPlan.isRelax) {
    ...
    if (!CONFIG.enableAnimations) {
      ...
      // (sinon le changement de mode n'est jamais transmis aux invités sync)
