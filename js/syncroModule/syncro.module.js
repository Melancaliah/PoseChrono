(function initPoseChronoSyncroModule(globalScope) {
  ...
  function renderParticipantsTooltip(input = {}) {
    ...
    guests.forEach((guest) => {
      ...
      const textGroupEl = documentRef.createElement("span");
      textGroupEl.className = "sync-session-participant-text";
      ...
      const syncStateLabels = {
        ready: getText("sync.participantSyncReady", "Poses téléchargées ✓"),
        syncing: getText("sync.participantSyncDownloading", "Téléchargement en cours…"),
        missing: getText("sync.participantSyncMissing", "En attente"),
      };
      sublabelEl.textContent =
        syncStateLabels[safeGuest.syncState] || syncStateLabels.missing;
      ...
      itemEl.appendChild(textGroupEl);
  ...
  // ─── Utilitaire interne : formate une taille en octets ───────────────────
  ...
  /**
   * Met à jour la barre d'info pack (#sync-session-pack-info) dans la modal sync.
   *
   * Hôte    → affiche "✓ N fichier(s) téléversé(s) · X Mo" quand le media pack est uploadé.
   * Invité  → affiche le nombre de fichiers disponibles, ou un avertissement si l'hôte
   *            n'a pas encore téléversé.
   */
  function updatePackStatusUi(modal, state, options = {}) {
    if (!modal) return false;
    const infoEl = modal.querySelector("#sync-session-pack-info");
    ...
    const filesCount = Math.max(
      0,
      Number(state?.sessionMediaMeta?.filesCount || 0) || 0,
    );
    const totalBytes = Math.max(
      0,
      Number(state?.sessionMediaMeta?.totalBytes || 0) || 0,
    );
    const mediaRefsCount = Math.max(
      0,
      Number(state?.sessionPackMeta?.mediaRefsCount || 0) || 0,
    );
    const hasMedia = filesCount > 0;
    // L'upload est terminé quand le pack final (avec refs) a été publié
    // ET qu'au moins un fichier est présent sur le serveur.
    // On ne requiert pas filesCount >= mediaRefsCount pour éviter les faux-négatifs
    // si quelques uploads ont échoué mais la majorité est disponible.
    const uploadComplete = mediaRefsCount > 0 && filesCount > 0;

    infoEl.classList.remove("is-ready", "is-waiting", "is-new", "is-hint", "hidden");

    if (role === "host") {
      if (!hasMedia) {
        // Pas encore uploadé — afficher le warning limite si beaucoup d'images chargées
        const totalMediaCount = Math.max(
          0,
          Number(options.totalMediaCount || 0) || 0,
        );
        const SYNC_MAX = 300;
        if (totalMediaCount > SYNC_MAX) {
          infoEl.classList.add("is-hint");
          infoEl.textContent = getText(
            "sync.packInfoLimitHint",
            `ℹ️ Seules ${SYNC_MAX} images seront partagées (sur ${totalMediaCount})`,
            { max: SYNC_MAX, total: totalMediaCount },
          );
        } else {
          // Peu d'images → pas de warning, on cache simplement la barre
          infoEl.classList.add("hidden");
        }
        return true;
      }
      // Upload en cours ou terminé → afficher le compteur
      const sizeText = totalBytes > 0 ? ` · ${formatPackBytes(totalBytes)}` : "";
      infoEl.classList.add("is-ready");
      infoEl.textContent =
        "✓ " +
        getText(
          "sync.packInfoUploaded",
          `${filesCount} fichiers téléversés${sizeText}`,
          { count: filesCount, size: sizeText },
        );
    } else if (role === "participant") {
      if (uploadComplete) {
        const sizeText = totalBytes > 0 ? ` · ${formatPackBytes(totalBytes)}` : "";
        const currentPackHash = String(state?.sessionPackMeta?.hash || "").trim();
        const downloadedHash = String(options.downloadedPackHash || "").trim();
        const alreadyDownloaded = !!(downloadedHash && currentPackHash && downloadedHash === currentPackHash);
        const newPackAvailable = !!(downloadedHash && currentPackHash && downloadedHash !== currentPackHash);

        if (alreadyDownloaded) {
          // Pack déjà téléchargé et à jour
          infoEl.classList.add("is-ready");
          infoEl.textContent =
            "✓ " +
            getText(
              "sync.packInfoDownloaded",
              `${filesCount} fichiers téléchargés${sizeText}`,
              { count: filesCount, size: sizeText },
            );
        } else if (newPackAvailable) {
          // L'hôte a publié un nouveau pack depuis le dernier téléchargement
          infoEl.classList.add("is-new");
          infoEl.textContent =
            "↓ " +
            getText(
              "sync.packInfoNewAvailable",
              `Nouveau pack · ${filesCount} fichiers${sizeText}`,
              { count: filesCount, size: sizeText },
            );
        } else {
          // Jamais téléchargé : afficher le nombre de fichiers disponibles
          infoEl.classList.add("is-ready");
          infoEl.textContent =
            "↓ " +
            getText(
              "sync.packInfoAvailable",
              `${filesCount} fichiers disponibles${sizeText}`,
              { count: filesCount, size: sizeText },
            );
        }
      } else {
        // Pré-upload OU upload en cours → message d'attente unifié
        infoEl.classList.add("is-waiting");
        infoEl.textContent = getText(
          "sync.packInfoUploading",
          "En attente que l'hôte partage ses images…",
        );
      }
    } else {
      infoEl.classList.add("hidden");
