...
async function findLatestAppImage(directoryPath) {
  const entries = await fsp.readdir(directoryPath, { withFileTypes: true });
  const candidates = [];
  for (const entry of entries) {
    if (!entry.isFile()) continue;
...
async function main() {
  ...
  // Linux artifacts (optional — cross-build may fail on Windows without symlink privileges)
  const linux = await findLatestDirectory(/^PoseChrono_v.+_linux_T\d{2}-\d{2}_\d{2}$/);
  let linuxAppImage = null;
  if (linux) {
    linuxAppImage = await findLatestAppImage(linux.fullPath);
  }
  ...
  // Copy Linux AppImage if available
  let movedAppImagePath = null;
  if (linuxAppImage) {
    const linuxDest = path.join(targetDir, "desktop-linux");
    await fsp.mkdir(linuxDest, { recursive: true });
    movedAppImagePath = path.join(linuxDest, linuxAppImage.name);
    await fsp.copyFile(linuxAppImage.fullPath, movedAppImagePath);
  }
  ...
  const meta = {
    ...
    linuxAppImage: linuxAppImage ? linuxAppImage.name : null,
    linuxArtifactFolder: linux ? linux.name : null,
    eagleArtifact: eagle.name,
    eagleZip: eagleZip ? eagleZip.name : null,
    movedDesktopExeTo: path.relative(ROOT, movedExePath),
    movedLinuxAppImageTo: movedAppImagePath ? path.relative(ROOT, movedAppImagePath) : null,
