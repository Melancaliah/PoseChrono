...
const MAX_TOTAL_ROOMS = Math.max(
  1,
  Number(process.env.POSECHRONO_SYNC_MAX_ROOMS || 500) || 500,
);
const MAX_CONNECTIONS = Math.max(
  1,
  Number(process.env.POSECHRONO_SYNC_MAX_CONNECTIONS || 200) || 200,
);
...
async function createRoom(state, payload) {
  ...
  if (state.rooms.size >= MAX_TOTAL_ROOMS) throw new Error("server-room-limit");
...
function updateSessionState(state, payload) {
  ...
  if (canSharedPlayback) {
    // Shared playback: broadcast control signal without mutating room state.
    // The host's full state (mediaOrderKeys, etc.) must remain intact.
    const controlPayload = normalizeSharedPlaybackRequestPayload(normalizedPayload);
    const controlSignal = {
      ...controlPayload,
      revision: Number(room.sessionStateRevision) || 0,
      updatedAt: nowMs(),
      sourceClientId,
    };
    broadcastSessionEvent(state, sessionCode, {
      type: "session-state-updated",
      source: "shared-playback-request",
      sessionCode,
      state: controlSignal,
    });
...
function onNotify(state, ws, action, payload) {
  ...
  if (isClientRateLimited(clientState, nowMs())) {
    logSecurityEvent(ws, clientState, "rate-limited", {
      action: `notify:${String(action || "").trim()}`,
    });
    return;
  }
...
async function main() {
  ...
  wss.on("connection", (ws) => {
    if (wss.clients.size > MAX_CONNECTIONS) {
      console.warn(`[sync-relay] max connections reached (${MAX_CONNECTIONS}), rejecting`);
  ...
  _activeInstance = { server, wss, shutdown };
...
// Instance active du serveur (permet l'arret depuis l'exterieur via require)
...
// Exposer stopServer pour l'arret in-process
...
if (require.main === module) {
  ...
  // Charge comme module : lancer le serveur automatiquement
  main().catch((error) => {
    console.error("[sync-relay] failed:", error);
  });
}
