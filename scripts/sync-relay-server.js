...
const DEFAULT_HOST = process.env.POSECHRONO_SYNC_RELAY_HOST || "127.0.0.1";
...
const MAX_SESSION_STATE_SERIALIZED_BYTES = Math.max(
  4096,
  Number(process.env.POSECHRONO_SYNC_MAX_STATE_BYTES || 900000) || 900000,
);
...
const MAX_MEDIA_ORDER_KEYS = Math.max(
  1,
  Number(process.env.POSECHRONO_SYNC_MAX_MEDIA_ORDER_KEYS || 12000) || 12000,
);
...
const MAX_SESSION_PACK_BYTES = Math.max(
  4096,
  Number(process.env.POSECHRONO_SYNC_MAX_SESSION_PACK_BYTES || 2 * 1024 * 1024) ||
    2 * 1024 * 1024,
);
